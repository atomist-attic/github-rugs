...
class ReactCommitCommentCommand implements HandleCommand {
    ...
    handle(ctx: HandlerContext): Plan {
        let plan = new Plan();
        let execute = exec("react-github-commit-comment", this)
        plan.add(wrap(execute,`Successfully reacted with :${this.reaction}: to ${this.owner}/${this.repo}/${this.sha1}/comments/${this.commentId}`,this))
        return plan;
    }
}
